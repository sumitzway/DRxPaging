 ...
 *
 * This allows the diff of a DRxPagedList, e.g.:
 ...
 * consideration.
 *
 * @see DRxPagedStorage#computeLeadingNulls()
 * @see DRxPagedStorage#computeTrailingNulls()
 */
class DRxPagedStorageDiffHelper {
    private DRxPagedStorageDiffHelper() {
    }

    static <T> DiffUtil.DiffResult computeDiff(
            final DRxPagedStorage<T> oldList,
            final DRxPagedStorage<T> newList,
    ...
    static <T> void dispatchDiff(ListUpdateCallback callback,
        ...
        if (leadingNew != 0) {
            diffResult.dispatchUpdatesTo(new DRxPagedStorageDiffHelper.OffsettingListUpdateCallback(leadingNew, callback));
        } else {
            diffResult.dispatchUpdatesTo(callback);
        }
    }
    ...
    static int transformAnchorIndex(@NonNull DiffUtil.DiffResult diffResult,
                                    @NonNull DRxPagedStorage oldList, @NonNull DRxPagedStorage newList, final int oldPosition) {
        final int oldOffset = oldList.computeLeadingNulls();
